...
4. ⚠️ **MatchupArenaOverlay.swift** - NEEDS FIX (converted to modal but anti-pattern code remains)
...
### 2. MatchupArenaOverlay.swift ⚠️ CLEANUP NEEDED

**Status:** Already converted to `.fullScreenCover()` but anti-pattern code remains
**Location:** Lines 16-17 (state), 103-110 (anti-pattern function)
...
private func handleFocusAnchorChange(newValue: MatchupFocusAnchor?) {
    ...
    if let newValue {
        ...
        focusAnchor = lastFocus  // ❌ Manual reset (now obsolete)
    }
}
```

**Remediation Required:**
1. Remove `handleFocusAnchorChange` function (lines 103-110)
2. Remove state variables `lastFocus` and `suppressFocusReset` (lines 16-17)
3. Update `applyFocusModifiers` in MatchupArenaOverlay+ViewModifiers.swift to remove `onFocusChange` callback
4. Remove setup code that references suppressFocusReset (line 57, line 75)
...
2. **MEDIUM:** MatchupArenaOverlay.swift - Remove obsolete anti-pattern code
...
- [ ] MatchupArenaOverlay still contains focus properly
- [ ] MatchupArenaOverlay directional navigation works (if any remains)
...
- Previous fix: ThemePicker, MatchupArena, Analytics, TierListBrowser modal conversion
