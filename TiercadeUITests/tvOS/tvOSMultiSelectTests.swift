...
final class MultiSelectTests: TiercadeTvOSUITestCase {
    ...
    func test_MultiSelect_action_bar_shows_move_buttons() throws {
        ...
        var foundButtons = 0
        for buttonID in moveButtons where app.buttons[buttonID].exists {
            foundButtons += 1
        }
    ...
    func test_MultiSelect_focus_returns_to_grid_after_move() throws {
        ...
        if moveButton.exists {
            ...
            for i in 0..<min(cards.count, 10) where cards.element(boundBy: i).hasFocus {
                cardHasFocus = true
                break
            }
