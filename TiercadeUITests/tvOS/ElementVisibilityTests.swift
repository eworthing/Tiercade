...
/// Tests to investigate element visibility issues in UI test hierarchy
/// Trying different query types and accessibility configurations
final class ElementVisibilityTests: XCTestCase {
    ...
    func test_wait_for_action_bar_with_long_timeout() throws {
        ...
        if appeared {
            ...
            if anySelectButton.exists {
                print("Found button with 'select': label='\(anySelectButton.label)', id='\(anySelectButton.identifier)'")
            }
        }
    }
}
