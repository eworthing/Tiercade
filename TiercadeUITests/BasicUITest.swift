...
final class BasicUITest: XCTestCase {
    ...
    func test_FindActionBarElements() throws {
        ...
        if !multiSelectButton.exists {
            ...
            let multiButton = app.buttons.matching(NSPredicate(format: "label CONTAINS[c] 'multi' OR label CONTAINS[c] 'select'")).firstMatch
            print("Found multi/select button: \(multiButton.exists) - '\(multiButton.label)'")
        }
