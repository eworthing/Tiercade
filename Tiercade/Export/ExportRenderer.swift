...
enum ExportRenderer {
    ...
    struct Config {
        var maxSize: CGSize = .init(width: 4096, height: 4096)
        var rowHeight: CGFloat = 220
        var itemSize: CGSize = .init(width: 180, height: 180)
        var itemSpacing: CGFloat = 16
        var sectionSpacing: CGFloat = 24
        var contentInsets: EdgeInsets = .init(top: 24, leading: 24, bottom: 24, trailing: 24)
    ...
    -> Data? {
        ...
        guard let baseImage = renderer.cgImage else {
            return nil
        }
        ...
        if sizing.scale != 1.0 {
            ...
            guard let scaledImage = renderer.cgImage else {
                return nil
            }
...
extension ExportRenderer {
    fileprivate static func makeRenderer(context: Context, cfg: Config) -> ImageRenderer<AnyView> {
    ...
    fileprivate static func resolveSizing(for size: CGSize, targetSize: CGSize?, cfg: Config) -> SizingResult {
    ...
    fileprivate static func makePNGData(from image: CGImage) -> Data? {
        let data = NSMutableData()
        guard
