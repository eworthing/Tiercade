...
extension TierThemeEntity {
    fileprivate static func normalizeTiers(_ tiers: [TierColorEntity]) -> [TierColorEntity] {
        var rankedKeys = Set<String>()
        var hasUnranked = false

        return tiers.reduce(into: [TierColorEntity]()) { result, tier in
            if tier.isUnranked {
                guard !hasUnranked else {
                    return
                }
                hasUnranked = true
                result.append(tier)
            ...
            guard rankedKeys.insert(key).inserted else {
                return
            }
            result.append(tier)
        }
    ...
    fileprivate static func normalizedKey(for tier: TierColorEntity) -> String {
