    ...
    -> SeedRunResults {
        ...
        for seed in seedRing {
            do {
                ...
                let t0 = Date()
                let items = await (try? coordinator.uniqueList(query: query, targetCount: targetN, seed: seed)) ?? []
        ...
        let passAtN = Double(runs.count(where: { $0.ok })) / Double(seedRing.count)
        let medianIPS = median(runs.map(\.ips))

        let seedResults = runs.map(\.ok)
        logger(
