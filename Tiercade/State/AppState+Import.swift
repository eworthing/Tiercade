...
extension AppState {
    ...
    private func parseAndConvertJSON(_ jsonString: String) async throws(ImportError) -> JSONImportResult {
        ...
        if let explicitOrder = explicitOrder {
            for raw in explicitOrder {
                let trimmed = raw.trimmingCharacters(in: .whitespacesAndNewlines)
                guard !trimmed.isEmpty else { continue }
                guard trimmed.caseInsensitiveCompare("unranked") != .orderedSame else { continue }
                let lowercased = trimmed.lowercased()
                if let canonical = newTiers.keys.first(where: { $0.lowercased() == lowercased }) {
                    if newTiers[canonical] == nil {
                        newTiers[canonical] = []
                    }
                } else {
                    newTiers[trimmed] = []
    ...
    private func resolveImportedTierOrder(from result: JSONImportResult) -> [String] {
        ...
        if let explicit = result.explicitTierOrder {
            ...
            for raw in explicit {
                ...
                guard let resolved = availableLookup[lowercased] else { continue }
