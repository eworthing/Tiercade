...
extension SystemPromptTester {
    ...
    static func buildLogSummary(results: [TestResult]) -> String {
        ...
        Passed: \(results.count(where: { !$0.hasDuplicates && !$0.insufficient }))
        Duplicates Only: \(results.count(where: { $0.hasDuplicates && !$0.insufficient }))
        Insufficient Only: \(results.count(where: { !$0.hasDuplicates && $0.insufficient }))
        Both Failures: \(results.count(where: { $0.hasDuplicates && $0.insufficient }))
        ================================================================================
        """
    }
