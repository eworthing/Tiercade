...
public enum HistoryLogic {
    ...
    public static func undo<T>(_ h: History<T>) -> History<T> {
        guard canUndo(h) else { return h }
        return History(stack: h.stack, index: h.index - 1, limit: h.limit)
    }

    public static func redo<T>(_ h: History<T>) -> History<T> {
        guard canRedo(h) else { return h }
        return History(stack: h.stack, index: h.index + 1, limit: h.limit)
    }
